...
module.exports = {
    getMain: async (req, res) => {
        const clickedPage =
            req.query.clickedPageNum === undefined
                ? 1
                : req.query.clickedPageNum;
        ...
        await sharedListData(searchInfo)
            .then((data) => {
                if (data.page === 1)
                    res.status(200).render("../views/board/list.ejs", data);
                /* 페이지 버튼 클릭에 따른 동적인 렌더링을 위해 별도의 ejs파일 render */ else
                    res.status(200).render(
                        "../views/board/pagingList.ejs",
                        data
                    );
            })
            .catch(() => {
    ...
    getUpdate: (req, res) => {
        ...
        if (req.session._id) {
            ...
            service
                ...
                .catch((reason) => {
                    res.status(reason).send(
                        "<script>alert('글쓴이만 수정할 수 있습니다'); location.href = document.referrer;</script>"
                    );
                });
        } else {
            res.status(401).send(
